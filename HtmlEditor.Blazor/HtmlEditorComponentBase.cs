...
{
    /// <summary>
    ///   Base class of HtmlEditor Blazor components.
    /// </summary>
    public class HtmlEditorComponentBase : ComponentBase, IDisposable
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="HtmlEditorComponentBase"/> is visible. Invisible components are not rendered.
        /// </summary>
        ...
        private DotNetObjectReference<HtmlEditorComponentBase> reference;

        /// <summary>
        /// Gets the reference for the current component.
        /// </summary>
        /// <value>The reference.</value>
        protected DotNetObjectReference<HtmlEditorComponentBase> Reference
        {
            get
            {
                if (reference == null)
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender || visibleChanged)
            {
                ...
                if (Visible)
                {
                    ...
                    {
                        await JSRuntime.InvokeVoidAsync("HtmlEditor.addContextMenu", UniqueID, Reference);
                    }

                    if (MouseEnter.HasDelegate)
                    {
                        await JSRuntime.InvokeVoidAsync("HtmlEditor.addMouseEnter", UniqueID, Reference);
                    }

                    if (MouseLeave.HasDelegate)
                    {
                        await JSRuntime.InvokeVoidAsync("HtmlEditor.addMouseLeave", UniqueID, Reference);
                    }
                }
            }
        }
        ...
        [JSInvokable("HtmlEditorComponentBase.RaiseContextMenu")]
        ...
        /// </summary>
        [JSInvokable("HtmlEditorComponentBase.RaiseMouseEnter")]
        ...
        /// </summary>
        [JSInvokable("HtmlEditorComponentBase.RaiseMouseLeave")]
        ...
        public virtual void Dispose()
        {
            ...
            if (IsJSRuntimeAvailable)
            {
                if (ContextMenu.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("HtmlEditor.removeContextMenu", UniqueID);
                }

                if (MouseEnter.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("HtmlEditor.removeMouseEnter", UniqueID);
                }

                if (MouseLeave.HasDelegate)
                {
                    JSRuntime.InvokeVoidAsync("HtmlEditor.removeMouseLeave", UniqueID);
                }
            }
        }
