...
{
    /// <summary>
    /// HtmlEditorUpload component.
    /// </summary>
    /// <example>
    /// <code>
    /// &lt;HtmlEditorUpload Url="upload/single" Progress=@(args => OnProgress(args, "Single file upload"))/&gt;
    /// @code {
    ...
    /// </code>
    /// </example>
    public partial class HtmlEditorUpload : HtmlEditorComponentBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="HtmlEditorUpload"/> upload is automatic.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="HtmlEditorUpload"/> is multiple.
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets a value indicating whether this <see cref="HtmlEditorUpload"/> is disabled.
        /// </summary>
        ...
        ClassList ChooseClassList => ClassList.Create("html-editor-fileupload-choose html-editor-button")
        ...
        ClassList ButtonClassList => ClassList.Create("html-editor-button html-editor-button-icon-only btn-light")
                                              .AddDisabled(Disabled);
        ...
        {
            await JSRuntime.InvokeAsync<string>("HtmlEditor.upload", fileUpload, Url, Multiple, false, ParameterName);
        }
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            ...
            if (firstRender || visibleChanged)
            {
                ...
                if (Visible)
                {
                    await JSRuntime.InvokeVoidAsync("HtmlEditor.uploads", Reference, Id);
                }
            }
        }
        ...
        {
            files.Remove(file);
            await JSRuntime.InvokeVoidAsync("HtmlEditor.removeFileFromUpload", fileUpload, file.Name);
            await Change.InvokeAsync(new UploadChangeEventArgs() { Files = files.Select(f => new FileInfo() { Name = f.Name, Size = f.Size }).ToList() });
        }
        ...
        [JSInvokable("HtmlEditorUpload.OnChange")]
        ...
        [JSInvokable("HtmlEditorUpload.OnProgress")]
        ...
        [JSInvokable("HtmlEditorUpload.OnComplete")]
        ...
        [JSInvokable("HtmlEditorUpload.OnError")]
